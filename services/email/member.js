...
exports.sendNewMemberInviteEmail = async (
  ...
  return await sendFrostmailEmail(
    MemberEmail,
    '🚀 Connect to EngageGPT - Member Invitation',
    html
  );
};
...
exports.sendPostStatusEmail = async (
  ...
  return await sendFrostmailEmail(
    ...
    isSuccess
      ...
      : '⚠️ LinkedIn Post Failed to Publish',
    html
  );
};
...
exports.sendExtensionConnectedConfirmation = async (user) => {
  ...
  return await sendFrostmailEmail(
    user.email,
    '✅ EngageGPT Extension Connected Successfully',
    html
  );
};
...
exports.sendOnboardingCompleteEmail = async (user) => {
  ...
  return await sendFrostmailEmail(
    user.email,
    '🎉 Onboarding Complete – Let the LinkedIn Magic Begin!',
    html
  );
};
...
exports.sendMilestoneEmail = async (user) => {
  ...
  const templateName = isSpecial
    ? 'milestones/special_milestone'
    : 'milestones/regular_milestone';
  ...
  const subject = isSpecial
    ? `🎉 ${daysActive} Days of EngageGPT - You're a Star!`
    : `👏 ${daysActive} Active Days – Keep Going!`;
